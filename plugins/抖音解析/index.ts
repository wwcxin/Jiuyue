...
export default definePlugin({
    name: '抖音解析',
    version: '1.0.0',
    desc: '抖音视频解析插件 - 支持无水印视频下载，支持视频直链获取',
        ...
        ctx.handleMessage(async e => {
            ...
            try {
                ...
                for (const url of urls) {
                    ...
                    const processingMsg = await ctx.respond(e, ['正在解析抖音视频，请稍候...'])
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (!result.isDirectLink) {
                                await ctx.respond(e, [Structs.video(result.url)])
                            } else {
                        ...
                        await ctx.respond(e, [`视频解析失败: ${error.message}`])
                    } finally {
                        if (processingMsg && processingMsg.message_id && e.group_id) {
                            await ctx.recallMsg(processingMsg.message_id)
                        }
                    }
                }
            } catch (error) {
